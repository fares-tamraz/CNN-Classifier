#!/usr/bin/env python3
"""
Train v5 model on 3-class dataset (good/faulty/no_cap).

Key improvements over v4:
1. Simplified 3-class task (easier to learn)
2. More balanced classes (38%/36%/26%)
3. Enhanced augmentation with Mixup
4. Cosine annealing learning rate
5. Label smoothing for better generalization
6. Ensemble-ready architecture
"""
...
def build_simple_cnn(input_shape, num_classes, dropout=0.4):
    """Build an optimized simple CNN for 3-class classification."""
    
    inputs = tf.keras.Input(shape=input_shape)
    
    # Data augmentation layer (applied during training)
    x = tf.keras.layers.RandomRotation(0.15)(inputs)
...
class CosineAnnealingScheduler(tf.keras.callbacks.Callback):
    ...
    def __init__(self, initial_lr, min_lr, epochs_per_cycle, verbose=0):
        super().__init__()
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        lr = self.min_lr + 0.5 * (self.initial_lr - self.min_lr) * \
             (1 + np.cos(np.pi * cycle_position / self.epochs_per_cycle))
        self.model.optimizer.learning_rate.assign(lr)
        if self.verbose:
            print(f"\nEpoch {epoch+1}: CosineAnnealing LR = {lr:.2e}")
...
def main():
    print("="*70)
    print("V5 3-CLASS MODEL TRAINING")
    print("="*70)
    
    # Configuration
    data_dir = "data/processed/cls_3class_crops"
    out_dir = Path("models/v5")
    ...
    train_ds, val_ds, test_ds, class_names = load_datasets(
        ...
        augment=False,  # We handle augmentation in the model
    )
